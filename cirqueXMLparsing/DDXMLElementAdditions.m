...
/**
 * Returns all children elements in a dictionary
 **/
- (NSDictionary *)childrenAsDictionary {
	...
	for (DDXMLElement *e in children) {
		if ([result objectForKey:[e name]]) {
			id obj = [result objectForKey:[e name]];
			NSMutableArray *arr = [NSMutableArray array];
			
			if ([obj isKindOfClass:[NSArray class]]) {
				[arr addObjectsFromArray:obj];
				[arr addObject:[e stringValue]];
				[result setObject:[NSArray arrayWithArray:arr] forKey:[e name]];
			}
			else {
				[arr addObject:obj];
				[arr addObject:[e stringValue]];
			...
			[result setObject:[e stringValue] forKey:[e name]];
		}
	}
	return result;
}
